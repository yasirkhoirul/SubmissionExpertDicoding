...
class _TvSeriesDetailPageState extends State<TvSeriesDetailPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      context.read<GetDetailTvSeriesBloc>().add(
        OnDetailTvSeriesE(widget.id.toInt()),
      );
      context.read<GetDetailTvSeriesBloc>().add(
        IsAddedOnWatchList(widget.id.toInt()),
      );
      context.read<GetRecomedationDetailTvCubit>().getRecomendation(
        widget.id.toInt(),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: BlocBuilder<GetDetailTvSeriesBloc, GetDetailTvSeriesState>(
        builder: (context, state) {
          if (state.tvseriesstatus == RequestState.Loading) {
            ...
            return SafeArea(
              child: Center(child: Text("error ${state.detailmessage}")),
            );
                              ...
                              child: Container(
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(data.name, style: kHeading5),
                                      FilledButton(
                                        onPressed: () {
                                          if (state.watchliststatus) {
                                            context
                                                .read<GetDetailTvSeriesBloc>()
                                                .add(OnRemoveWatchListTv(data));
                                          } else {
                                            context
                                                .read<GetDetailTvSeriesBloc>()
                                                .add(OnAddWatchListTv(data));
                                          }
                                        },
                                        child: Row(
                                      ...
                                      SizedBox(height: 5),
                                      Text("Overview", style: kHeading5),
                                      Expanded(child: Text(data.overview)),
                                      SizedBox(height: 5),
                                      Text("Recomendation", style: kHeading5),
                                      BlocBuilder<
                                        GetRecomedationDetailTvCubit,
                                        GetRecomedationDetailTvState
                                      >(
                                        builder: (context, state) {
                                          if (state
                                              is GetRecomendationDetailTvLoading) {
                                            return Row(
                                              mainAxisAlignment:
                                              ...
                                              is GetRecomendationDetailTvError) {
                                            return Row(
                                              mainAxisAlignment:
                                                  MainAxisAlignment.center,
                                              crossAxisAlignment:
                                              ...
                                              is GetRecomendationDetailTvLoaded) {
                                            return SizedBox(
                                              height: 200,
